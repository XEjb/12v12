...
function VotingToKickShowVoting(data) {
	...
	$.CreatePanelWithProperties(`DOTAScenePanel`, HUD.MODEL_ROOT, "VotingToKickVotingHeroModel", {
...
function VotingToKickVoteYes() {
	GameEvents.SendCustomGameEventToServer("voting_to_kick:vote_yes", {});
...
function VotingToKickInitVoting(reason) {
	GameEvents.SendCustomGameEventToServer("voting_to_kick:reason_picked", { reason: reason });
}
function ReportVoting() {
	HUD.CONTEXT.SetHasClass("BPlayerReported", true);
	GameEvents.SendCustomGameEventToServer("voting_to_kick:report", {});
}

(() => {
	GameEvents.SendCustomGameEventToServer("voting_to_kick:check_state", {});
